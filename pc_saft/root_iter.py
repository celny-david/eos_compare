...
class d_root:
    ...
    def get_root1(self, est: float, d_root0: Optional[float]) -> Optional[float]:
        ...
        def df2_root1(x, y0):
            ...
            return ((self.rho - y0) ** 2 * self.ppp + 2 * (
                        y0 - self.rho) * self.pp + 2 * self.p - 2 * self.target_pressure) / (self.rho - y0) ** 3
    ...
    def get_root2(self, est: float, d_root0: Optional[float], d_root1: Optional[float]) -> Optional[float]:
        ...
        def df_root2(x, y0, y1):
            ...
            return ((self.rho - y0) * (self.rho - y1) * self.pp + self.p * (
                        -2 * self.rho + y0 + y1) + self.target_pressure * (2 * self.rho - y0 - y1)) / (
                               (self.rho - y0) ** 2 * (self.rho - y1) ** 2)
        ...
        def df2_root2(x, y0, y1):
            ...
            return -(-(self.rho - y0) ** 2 * (self.rho - y1) ** 2 * self.ppp + 2 * (self.rho - y0) * (
                        self.rho - y1) * self.pp * (2 * self.rho - y0 - y1) - 2 * self.p * (
                                 3 * self.rho ** 2 - 3 * self.rho * (
                                     y0 + y1) + y0 ** 2 + y0 * y1 + y1 ** 2) + 2 * self.target_pressure * (
                                 3 * self.rho ** 2 - 3 * self.rho * (y0 + y1) + y0 ** 2 + y0 * y1 + y1 ** 2)) / (
                               (self.rho - y0) ** 3 * (self.rho - y1) ** 3)
...
class pfrac_root:
    ...
    def __init__(self, p_function: Callable, pfrac_to_dens: Callable, target_pressure: float,
                 precision_digits: int = 14):
        self.get_p = p_function  # has to be the pc_saft_interface type of function returning: p,pp,ppp =f(rho)
        self.pfrac_to_dens = pfrac_to_dens  # has to be the pc_saft_interface type of function returning density for pfrac input
        self.target_pressure = target_pressure
        # to bypass repeated calls of function last calculated values are remebered and reused if possible
        self.pfrac = None
        self.p = None  # pressure, output from the eos
    ...
    def _clean_root(self, root: float, digits: Optional[int] = None) -> float:
        """ makes sure to get rid of the .99999 noise in numbers """
        if digits is None:
            digits=self.prec_digits
        
        if type(root) is np.ndarray:
            return round(root[0], ndigits=digits)
        else:            
            return round(root, ndigits=digits)


    def get_root0(self, est: Optional[float] = 1e-10) -> Optional[float]:
        """function that returns the first density root of the p_function"""

        def f_root0(x):
            self.pfrac = x
            self.p, self.pp, self.ppp = self.get_p(pfrac=x, is_silent=self.is_silent)
            self.pcall[0] += 1
            return self.p - self.target_pressure

        # BUG untill derivatives according to pfrac are available
        # def df_root0(x):
        #     if x == self.pfrac:
        #         pass
        #     else:
        ...
        try:
            pfrac_root0, optim_report = newton(f_root0, est,
                                               # fprime=df_root0,
                                               # fprime2=df2_root0,
                                               full_output=True,
                                               maxiter=self.maxiter,
            ...
            if optim_report.converged is False:
                ...
                # print(d_root0)
                # print(optim_report)
                pfrac_root0 = self._clean_root(pfrac_root0)
                d_root0 = self.pfrac_to_dens(pfrac_root0)
                # print(f"root0: {pfrac_root0}, {d_root0}, {self.pcall[0]}") # DEBUG
            return d_root0
    ...
    def get_root1(self, est: Optional[float] = None) -> Optional[float]:
        """function that returns the second density root of the p_function"""

        return None

    def get_root2(self, est: Optional[float] = 0.4) -> Optional[float]:
        """function that returns the third density root of the p_function"""
        ...
        try:
            pfrac_root2, optim_report = newton(f_root0, est,
                                               # fprime=df_root0,
                                               # fprime2=df2_root0,
                                               full_output=True,
                                               maxiter=self.maxiter,
            ...
            if optim_report.converged is False:
                ...
                pfrac_root2 = self._clean_root(pfrac_root2)
                d_root2 = self.pfrac_to_dens(pfrac_root2)
                # print(f"root2: {pfrac_root2}, {d_root2}, {self.pcall[2]}") # DEBUG
            return d_root2
